import { 
  ...
  namesOnly, 
  makeStrings,
  readyToPutInTheDOM } from './array-map.js';
...
test('turn array of numbers into strings', () => {
  const input = [1, 2, 3, 4, 5];
  const output = stringItUp(input);
  expect(output).toEqual(['1', '2', '3', '4', '5']);
});

test('capitalize an array of names', () => {
...
test('Output only the names and nothing else', () => {
  const input = [
    {
      name: 'Angelina Jolie',
      age: 80
